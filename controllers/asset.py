...
def asset_create():
    ...
    return dict(
        ...
        asset_status_list=get_combo_values("asset_status") or [],
        asset_condition_list=get_combo_values("asset_condition") or []
    )
...
def get_brands_by_type():
    ...
    if not asset_type: return dict(results=[])
...
def get_models_by_type_brand():
    ...
    if not asset_type or not asset_brand: return dict(results=[])
...
def asset_edit():
    ...
    return dict(
        ...
        data = {k: (v or "") for k, v in main_row.as_dict().items()},
        asset_type_list = distinct_fields.get('asset_type', []),
        asset_brand_list = distinct_fields.get('asset_brand', []),
        asset_model_list = distinct_fields.get('asset_model', []),
        asset_status_list = get_combo_values("asset_status", as_dict=True) or [],
        asset_condition_list = get_combo_values("asset_condition", as_dict=True) or []
    )
...
def asset_update():
    ...
    # Helper to safely get text fields
    def get_text(key):
...
def asset_get_data():
    ...
    where_clauses = ["1=1"]
    if asset_name: where_clauses.append(f"asset_name = '{asset_name}'")
    if asset_type: where_clauses.append(f"asset_type = '{asset_type}'")
    if asset_status: where_clauses.append(f"asset_status = '{asset_status}'")
    if user_name: where_clauses.append(f"user_name = '{user_name}'")
...
def get_purchase_details_map():
    ...
    sql = f"""
        ...
        WHERE pd.purchase_head_id = '{purchase_head_id}'
